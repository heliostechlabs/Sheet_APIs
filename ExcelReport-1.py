...
def read_data_from_source_file(source_file, sheet_name, column_index):
    ...
    for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row, min_col=column_index, max_col=4, values_only=True):
    ...
    columns = list(dict.fromkeys(column_data))
    
    # print (data_dict)
    add_columns_at_location('C:\\Users\\brady\\Desktop\\test.xlsx', 'Sheet1', 3, 2, columns, bills_dict, data_dict)
        
    return column_data


def add_columns_at_location(file_path, sheet_name, column_index, num_columns, column_names, bills_dict, data_dict):
...
file_path = 'C:\\Users\\brady\\Desktop\\test.xlsx'
